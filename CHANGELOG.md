...
Evidence pass (2026-09-25) on `seo-maps`, its agent and the four `maps-*.md` references. Every endpoint, limit and price was checked against DataForSEO's docs and pricing pages, Google Maps Platform docs and terms, GBP Help, the OSMF Nominatim policy, the Overpass wiki and manual, Geoapify pricing and docs, and Local Falcon's SoLV page. Sources and removed claims are in `skills/seo/references/local-eeat-evidence.md` (new "Maps APIs" table).
...
### Changed
- **DataForSEO v3 audit** (2026-09-25): re-checked the migration against `dataforseo-mcp-server` 3.1.1 (npm `latest`, 2026-08-25; tools `api_request`, `docs_search`, `docs_index`, `docs_list_sections`). All 21 files in `skills/`, `agents/` and `extensions/` that name v2 tools pair them with the v3 `api_request` form; all 103 `/v3/...` paths resolve to endpoints in https://docs.dataforseo.com/v3/llms.txt (the `filters` doc pages map to `available_filters` API paths). The LLM Mentions renames (`search_mentions`, `top_mentioned_domains`, `top_mentioned_pages`, `target_metrics`, `multi_target_metrics`) are confirmed there. Live prices in `extensions/dataforseo/README.md` and `extensions/dataforseo/docs/DATAFORSEO-SETUP.md` match the nine pricing pages, the $1 trial credit and the $50 minimum top-up. Fixed: `docs/MCP-INTEGRATION.md` said v2 had 79 tools (2.9.13, the last 2.x, has 90; 2.8.10 had 80); `DATAFORSEO-SETUP.md` gained the Google Images SERP price row. New `tests/test_dataforseo_v3.py`: the catalog maps all 90 v2 tools, every file naming a v2 tool also names `api_request` and only uses catalog names, both installers pin `dataforseo-mcp-server@3`, and the extension mirror matches `skills/` and `agents/`. Sources: https://registry.npmjs.org/dataforseo-mcp-server, https://github.com/dataforseo/mcp-server-typescript#readme, https://dataforseo.com/pricing
- **hemsida** (claude.ai package, 2.0.0): now a website-creation orchestrator with nine phases (0 brief and fact sheet, 1 strategy, 2 content, 3 build, 4 technical SEO and AI search, 5 local, 6 conversion and measurement, 7 pre-launch QA gate, 8 launch and follow-up), a renovate mode (compare repo vs live site before any change or deploy, scan for leaked secrets, baseline, 301 map, pinned deploy) and the review mode. It delegates to claude-seo `/seo` commands (Claude Code only), marketingskills and installed claude.ai skills when present and falls back to its own checklists when not; its evidence rules override delegated output. New references: `skapa-fas.md`, `skill-karta.md`, `faktablad-mall.md`, `qa-grind.md`, `renovera.md`, `exempel-bygge.md`.
- **DataForSEO: migrated to dataforseo-mcp-server 3.x**, which replaced the per-endpoint v2 tools with `api_request`, `docs_search`, `docs_index` and `docs_list_sections` (3.0.0 2026-08-11, current 3.1.1 2026-08-25). Skills call `api_request` with verified v3 endpoint paths and keep v2 tool names as fallback; detection reports "DataForSEO MCP not detected" instead of silently falling back (agricidaniel/claude-seo#319). `tool-catalog.md` is now a v2 name to v3 endpoint map for all 90 v2 tools. LLM Mentions uses the renamed endpoints. Installers pin `dataforseo-mcp-server@3`, require Node.js 22+, use `DATAFORSEO_LOGIN`, drop `ENABLED_MODULES`. `field-config.json` rewritten in the v3 format (v3 rejected the old file). Extension mirror synced. Organic SERP default depth 10; Images SERP depth max 200. Sources: https://registry.npmjs.org/dataforseo-mcp-server, https://github.com/dataforseo/mcp-server-typescript, https://docs.dataforseo.com/v3/llms.txt
...
- **seo-maps evidence pass** (SKILL.md, `agents/seo-maps.md`, `maps-gbp-checklist.md`, `maps-geo-grid.md`, `maps-api-endpoints.md`, `maps-free-apis.md`): evidence rule added ([V] only for Critical/High; weights, SoLV bands, velocity and fake-review patterns labeled heuristic, Medium at most); a "Finding Phrasing" wrong-vs-right table with invented data. GBP checklist: each criterion tagged [V]/[H]/[H survey]; description max 750 characters, up to 20 service areas, post types updates/offers/events, local phone guidance and "more likely to show up" for verified profiles now cite GBP Help (https://support.google.com/business/answer/7091, /3038177, /3039617, /9157481, /7342169); Whitespark 2026 (#1 Primary GBP Category, #5 open at search) and BrightLocal LCRS 2026 (80% likely to use a business that responds to all reviews) labeled as surveys. Q&A field dropped because Google discontinued its Q&A API on 2025-11-03: 24 fields, 48 points, subtotals 16/16/16. Score band "Critical" renamed "Very poor", SoLV band "Critical" renamed "Very weak". SoLV defined per Local Falcon (https://www.localfalcon.com/knowledge-base/kb32-what-is-solv-how-to-interpret-and-use-the-metric). NAP mismatches capped at Medium/Low. Tier 2 note now quotes the Maps Platform terms (place_id cacheable; Places lat/lng up to 30 days, https://cloud.google.com/maps-platform/terms/maps-service-terms).
- **Tier 1 detection** in seo-maps and its agent accepts `api_request` from dataforseo-mcp-server 3.x (npm latest 3.1.1, 2026-08-25), which `extensions/dataforseo/install.sh` installs unpinned, as well as the v2 tool `business_data_business_listings_search`. Source: https://www.npmjs.com/package/dataforseo-mcp-server
- **Free APIs**: Nominatim section follows the OSMF policy (1 req/s, no grid reverse queries, bulk "not encouraged", 4 req/min for scheduled scripts, the LLM clause; https://operations.osmfoundation.org/policies/nominatim/). Overpass: regular-use limit (divide by 100), User-Agent on every curl example, 30 s pause on 429/406, slot count from `/api/status` (https://wiki.openstreetmap.org/wiki/Overpass_API). Geoapify: credit rule, attribution, OSM as Places source (https://www.geoapify.com/pricing, https://apidocs.geoapify.com/docs/places/). Example User-Agent updated from 1.7.0 to 1.8.1.
### Fixed
- **Extension MCP servers never loaded**: the DataForSEO, Firecrawl and Banana installers wrote `mcpServers` into `~/.claude/settings.json`, which Claude Code does not read MCP servers from; user-scope servers live in `~/.claude.json` (https://code.claude.com/docs/en/mcp). Installers now run `claude mcp remove` then `claude mcp add --env ... --transport stdio --scope user <name> -- npx -y <package>` for `dataforseo`, `firecrawl-mcp` and `nanobanana-mcp`; without the CLI they merge the entry into `~/.claude.json` (backup, atomic write, other keys untouched). Legacy `settings.json` entries are removed with a backup. Uninstallers mirror this. Shared helper: `extensions/claude_mcp_config.py` (secrets only via environment, never printed). Banana `setup_mcp.py`/`validate_setup.py` moved to `~/.claude.json`, no longer fail on `@latest` or unset `NANOBANANA_MODEL`; `BANANA-SETUP.md` key name corrected to `GOOGLE_AI_API_KEY`. PowerShell scripts reviewed but untested (no pwsh in CI).
- **Schema hook** blocks Swedish template tokens such as `[FÖRETAGSNAMN]`, `[Telefon]`, `[Ditt område]` (footnotes like `[1]` still pass).
- **DataForSEO tool names that never existed**: `serp_google_images_live_advanced` (seo-images), `dataforseo_backlinks_*` (seo-backlinks), `local_business_data` / `google_local_pack_serp` / `business_listings` (seo-local). Agents seo-dataforseo, seo-maps, seo-backlinks, seo-geo and seo-local now list `mcp__dataforseo` in `tools:`; their allowlists excluded all MCP tools (https://code.claude.com/docs/en/sub-agents).
- **DataForSEO cost tables** in `extensions/dataforseo/README.md` and its setup doc replaced with live prices from https://dataforseo.com/pricing (fetched 2026-09-25); old ranges were off by up to 10x.
...
- **DataForSEO costs and limits** (`maps-api-endpoints.md`, `maps-geo-grid.md`): Google Reviews cost $0.00075 per 10 reviews, not $0.003, and "place_id 4x cheaper" was wrong (Extended Reviews: $0.0015 vs $0.00225 per 20); My Business Info live costs $0.0054 ($0.0015 is the standard queue); Tripadvisor bills per 10 reviews, not 30; full-audit estimate ~$0.15/~$0.34 (was ~$0.13/~$0.33). Live calls carry one task each, so the advice to batch 49 live grid tasks in one request was wrong (batching needs `task_post`). The 2,000 calls/min limit does not cover Tripadvisor (110) or Trustpilot (30). Corrected field names (`phone`, `total_photos`, `work_time.work_hours`, `is_claimed`, `sort_by: relevant`), listings `limit` max 1000. Sources: https://dataforseo.com/pricing/business-data/google-reviews-api, https://dataforseo.com/pricing/business-data/business-data-api, https://dataforseo.com/pricing/serp/google-maps-serp-api, https://docs.dataforseo.com/v3/serp/google/maps/live/advanced/
- **Geoapify categories** `service.financial.accounting` and `commercial.vehicle.car_dealer` do not exist; replaced with documented ones. Nominatim `format=json` returns `class`, not `category`.
